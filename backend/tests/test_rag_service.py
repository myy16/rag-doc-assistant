...
class FakeRetriever:
    ...
    def retrieve(self, query, top_k=5, file_id=None, source_file=None):
        self.retrieve_calls.append((query, top_k, file_id, source_file))
        return [
            {
                "text": "Chunk 1 içerik",
    ...
    def fetch_documents(self, file_id=None, source_file=None):
        self.fetch_calls.append((file_id, source_file))
        return [
            {
                "text": "Chunk 1 içerik",
                "metadata": {"source_file": "dosya.pdf", "chunk_index": 0, "char_count": 14},
...
def test_answer_question_returns_sources_without_llm_when_no_context(monkeypatch):
    ...
    assert result["answer"] == "İlgili bağlam bulunamadı."
...
def test_summarize_documents_uses_retrieved_chunks(monkeypatch):
    service, _, fake_retriever, _ = build_service(monkeypatch)
    monkeypatch.setattr(service, "_call_groq", lambda prompt: "Özet üretildi")

    result = service.summarize_documents(source_file="dosya.pdf", max_chunks=1)

    assert fake_retriever.fetch_calls == [(None, "dosya.pdf")]
